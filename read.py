...
from hypo import hypothesis_testing_chi
...
def read_zero(df_dict):
    ...
    avg_mass = for_zero(mass_list[0], mass_list[1], mass_list[2], mass_list[3],
                        mass_list[4])
    return avg_mass
...
def chi_q(df_dict: dict, mas_zero: list):
    for x in df_dict.keys():
        ...
        for ind in range(8):
            ...
            print(*mas_chi_table_all[ind], sep=" & ",
                  end='\\\\ \\cline{2-7} \n')
            # print("\n")
...
def test_table(df_dict, mas_zero):
    ...
    plt.plot(x, sps.norm.pdf(x, loc=np.mean(column), scale=np.std(column)),
             color='red',
