...
class hypothesis_testing_chi:
    def __init__(self, sampling: list):
        self.n = len(sampling)
        self.distribution = sps.norm(loc=np.mean(sampling),
                                     scale=np.std(sampling))
    ...
    def test_hyp(self, alpha, flag):
        ...
        a = np.linspace(min(self.sampling) + 0.5, max(self.sampling) - 0.5,
                        k - 1)
        ...
        while i != len(a_) - 1:
            if i + 1 == len(a_) - 1:
                p_i = self.distribution.cdf(a_[-1]) - self.distribution.cdf(
                    a_[-2])
                if self.n * p_i >= 5:
                    i += 1
                    break
                ...
                k_ -= 1
                continue
            p_i = self.distribution.cdf(a_[i + 1]) - self.distribution.cdf(
                a_[i])
            if self.n * p_i >= 5:
                i += 1
                continue
        ...
        sqr_n_k_np_i = [round((n_k[i] - self.n * p[i]) ** 2, 2) for i in
                        range(k_)]
        chi_B_list = [
            round(((n_k[i] - self.n * p[i]) ** 2) / (self.n * p[i]), 2) for i in
            range(k_)]
        ...
        if flag == 1:
            print(
                f"{1} & ($-\infty,{'%.2f' % a_[1]}$] &{p[0]} & {np_i[0]}&{n_k[0]}&{n_k_np_i[0]}"
                f"&{sqr_n_k_np_i[0]}&{chi_B_list[0]}", end='\\\\ \\hline \n')
            for i in range(1, len(p) - 1):
                print(
                    f"{i + 1} & ({'%.2f' % a_[i]}, {'%.2f' % a_[i + 1]}] & {p[i]}&{np_i[i]}&{n_k[i]}&{n_k_np_i[i]}"
                    f"&{sqr_n_k_np_i[i]}&{chi_B_list[i]}",
                    end='\\\\ \\hline \n')
            print(
                f"{k_} & (${'%.2f' % a_[-2]},\infty$)&{p[-1]} & {np_i[-1]}&{n_k[-1]}&{n_k_np_i[-1]}"
                f"&{sqr_n_k_np_i[-1]}&{chi_B_list[-1]}", end='\\\\ \\hline \n')
            print(
                f"\sum &-&{round(sum(p), 2)} &{round(sum(np_i), 2)}&{round(sum(n_k), 2)}&{round(sum(n_k_np_i), 2)}"
                f"&{round(sum(sqr_n_k_np_i), 2)}&{round(sum(chi_B_list), 2)}",
                end='\\\\ \\hline \n')
